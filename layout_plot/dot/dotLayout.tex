\newcommand{\drawBlockMFMALayoutLarge}[3]{
  ...
  %% \waveSize: pre defined variable (32 or 64)
  %%
  ...
  \pgfmathsetmacro{\groups}{\waveSize/\nonKDim}
  \pgfmathsetmacro{\maxGID}{\groups-1}
  %% outVecSize = number of output elements per thread = nonKDim^2 / waveSize
  \pgfmathsetmacro{\outVec}{int(\nonKDim*\nonKDim/\waveSize)}
  \pgfmathsetmacro{\maxIVec}{\nonKDim*\nonKDim/256-1}
  \pgfmathsetmacro{\verbose}{#3}
  \foreach \iVec in {0,...,\maxIVec} {
    \coordinate (wave TL) at ($(block TL)+(\trans*\iVec*\groups*\outVec*\elem, -\nonTrans*\iVec*\groups*\outVec*\elem)$);
    \foreach \tg in {0,...,\maxGID}{
      ...
      \foreach \tid in {0,...,\maxTID} {
        ...
        \ifthenelse{\verbose=0}{
          ...
          ($(wave TL)+(\nonTrans*\tid*\elem+\tg*\trans*\outVec*\elem, -\trans*\tid*\elem-\tg*\nonTrans*\outVec*\elem)$)
          rectangle ++(\nonTrans*\elem+\trans*\outVec*\elem, -\nonTrans*\outVec*\elem-\trans*\elem);
        }{
...
\newcommand{\drawMFMAOperand}[5]{
  ...
  \pgfmathsetmacro{\nonKDim}{#1}
  \pgfmathsetmacro{\maxGID}{\waveSize/\nonKDim-1}
  \pgfmathsetmacro{\maxTID}{\nonKDim-1}
  ...
  \foreach \gp in {0,...,\maxGroupId}{
    \coordinate (group TL) at ($(mfma op TL)+(\gp*\kWidth*\waveSize*\elemW/\nonKDim*\opIdxB, -\gp*\kWidth*\waveSize*\elemW/\nonKDim*\opIdxA)$);
...
\newcommand{\drawMFMAInstr}[6]{
  ...
  %% Compute output vector size: vecSize = nonKDim * nonKDim / waveSize
  %% For 64-thread waves with nonKDim=16: 16*16/64 = 4
  %% For 32-thread waves with nonKDim=16: 16*16/32 = 8
  \pgfmathsetmacro{\outVecSize}{int(\mfmaNonKDim*\mfmaNonKDim/\waveSize)}
  \pgfmathsetmacro{\maxOutVecId}{\outVecSize-1}
  ...
  \ifthenelse{\mfmaTrans=0}{
    ...
    \foreach \vecId in {0,...,\maxOutVecId}{
      \draw ($(vec TL)+(0, -\vecId*\elem)$) rectangle ++(\elem, -\elem);
    }
    \draw [densely dotted] (block TL) -- ++(-3*\elem,0);
    \draw [densely dotted] ($(block TL)+(0, -\outVecSize*\elem)$) -- ++(-3*\elem,0);
    \node [scale=.8*\scale, above, rotate=90] at ($(vec TL)+(0, -.5*\outVecSize*\elem)$) {vec=\outVecSize$\times$\outType};
    ...
    \draw [densely dotted] ($(block TL)+(\outVecSize*\elem, 0)$) -- ++(0, 3*\elem);
    \node [scale=.8*\scale, above] at ($(vec TL)+(.5*\outVecSize*\elem, 0)$) {vec=\outVecSize$\times$\outType};

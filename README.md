...
The headline result: on `a16w16` (FP16, 4096×4096×8192), the naive Gluon baseline runs at **523 TFLOPS**. Nine versions later, it reaches **1571 TFLOPS** — a **~3.0× speedup**, every step motivated by a thread trace or a hardware counter. This repo is not a collection of finished kernels; it is the record of that journey, kept so readers can see *how* near-peak performance is built, not just what the final kernel looks like.

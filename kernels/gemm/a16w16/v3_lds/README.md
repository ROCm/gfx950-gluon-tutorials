...
## Motivation: How Do We Evaluate ds_read Performance?
...
## Three LDS Layouts: Design and Evaluation
...
### Raw Layout (No Swizzling, No Padding)
...
We can visualize the layout of operand A (256x64) and access pattern of `ds_read_b128` using:
```bash
...
```
The generated plot can be found at [v3_lds_no-swizzling.pdf](../images/v3_lds_no-swizzling.pdf).

From the plot — or from ATTViewer thread traces ([screenshot](../images/v3_lds_no-swizzling_att.png)) — we observe that each `ds_read_b128` experiences 4-way bank conflicts.
...
In summary, the raw layout is simple and register-efficient but suffers from severe bank conflicts.
...
We can visualize the same operand A with swizzling layout using:
```bash
python3 plot_layout.py lds \
  --tensorShape 256 64 \
  --kWidth 8 \
...
```
The generated plot can be found at [v3_lds_swizzling.pdf](../images/v3_lds_swizzling.pdf).

From thread traces ([screenshot](../images/v3_lds_swizzling_8-2-8_att.png)), 
we observe that ds_read_b128 now issues every 16 cycles in steady state. Bank conflicts are eliminated.
...
Swizzling therefore trades bank conflicts for increased complexity and register usage.
...
We can visualize the same operand A with padded layout using:
```bash
python3 plot_layout.py lds \
  --tensorShape 256 64 \
  --kWidth 8 \
...
```
The generated plot can be found at [v3_lds_padding_512-16.pdf](../images/v3_lds_padding_512-16.pdf).
Note that the layout plot tool can take the shared layout as-is :sunglasses:

Thread traces ([screenshot](../images/v3_lds_padding_512-16_att.png)) confirm that 
`ds_read_b128` returns to 16-cycle steady-state issue latency, indicating no bank conflicts.
...
Padding achieves both conflict-free access and minimal base VGPR usage
From the evaluation criteria established in Section 1, padding is the most balanced design among the three.
...
Future kernel versions will build on this foundation and integrate layout design with prefetching and scheduling control to achieve robust, high-performance implementations.

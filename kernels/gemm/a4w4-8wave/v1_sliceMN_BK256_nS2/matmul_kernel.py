...
"""
v1_sliceMN_BK256_nS2 -- 8-wave warp-pipeline MXFP4 (a4w4) GEMM, M/N-sliced.

This is the a4w4 (4-bit, MXFP4/e2m1) analogue of
a16w16-8wave/v1_sliceMN_BK64_nS2. The 8-wave skeleton -- 8 warps ([2,4] =
...
MIN_K = 4 * BLOCK_K
KERNEL_NAME = "v1_sliceMN_BK256_nS2"


@gluon.jit
def v1_sliceMN_BK256_nS2(
    a_ptr, b_ptr, c_ptr,  #
    ...
    smem_a_sc_t = gl.allocate_shared_memory(a_scales_ptr.type.element_ty, [nBuffers, BLOCK_M // 2, BLOCK_K // SCALE_GROUP_SIZE], sharedScaleLayout)
    smem_a_sc_b = gl.allocate_shared_memory(a_scales_ptr.type.element_ty, [nBuffers, BLOCK_M // 2, BLOCK_K // SCALE_GROUP_SIZE], sharedScaleLayout)
    smem_b_sc_l = gl.allocate_shared_memory(b_scales_ptr.type.element_ty, [nBuffers, BLOCK_N // 2, BLOCK_K // SCALE_GROUP_SIZE], sharedScaleLayout)
    smem_b_sc_r = gl.allocate_shared_memory(b_scales_ptr.type.element_ty, [nBuffers, BLOCK_N // 2, BLOCK_K // SCALE_GROUP_SIZE], sharedScaleLayout)
    ...
    offs_asm = (pid_m * BLOCK_M + gl.arange(0, BLOCK_M // 2, gl.SliceLayout(1, blocked_scales_half))) % M
    ...
    offs_bsn = (pid_n * BLOCK_N + gl.arange(0, BLOCK_N // 2, gl.SliceLayout(1, blocked_scales_half))) % N
    ...
    cdna4_async.buffer_load_to_shared(smem_b_sc_r.index(0), b_scales_ptr + b_sc_half_n, b_sc_offsets)
    cdna4_async.commit_group()
    ...
    cdna4_async.buffer_load_to_shared(smem_b_sc_r.index(1), b_scales_ptr + b_sc_half_n + b_sc_k, b_sc_offsets)
    cdna4_async.commit_group()

...
# ---------------------------------------------------------------------------
# Benchmark infrastructure (adapted from benchmark_fa_compare.py)
# ---------------------------------------------------------------------------

def _force_config(config):
    """Force gluon_attn_fwd to use a specific autotune config."""
    gluon_attn_fwd.configs = [config]
    gluon_attn_fwd.cache.clear()


def _restore_configs(all_configs):
    """Restore full autotune config list."""
    gluon_attn_fwd.configs = list(all_configs)
    gluon_attn_fwd.cache.clear()


def _run_kernel(B, HQ, HK, M, N, D, causal, layout, dtype):
    """Run the Gluon kernel once; return (output, reference)."""
    q, k, v, md = input_helper(B, HQ, HK, M, N, D, dtype, layout)
    if causal:
        md.need_causal()
    ...
    if layout == 'bhsd':
        ...
        o_ref = sdpa_reference(
            q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2),
            causal=causal, sm_scale=md.sm_scale
        ).transpose(1, 2)

    return o, o_ref, md


def find_best_accurate_config(B, HQ, HK, M, N, D, causal, layout, dtype,
                               atol=1e-3, rtol=1e-3, verbose=False):
    """Try each autotune config, keep accurate ones, benchmark, return best.

    Returns (tflops, ms, max_diff, config_name) or (None, None, inf, reason).
    """
    all_configs = list(gluon_attn_fwd.configs)
    accurate_configs = []

    for i, config in enumerate(all_configs):
        _force_config(config)
        try:
            o, o_ref, _ = _run_kernel(B, HQ, HK, M, N, D, causal, layout, dtype)
            passed, max_diff, _ = _check_output(o, o_ref, atol=atol, rtol=rtol)
        except Exception:
            passed, max_diff = False, float('inf')

        stages = config.kwargs.get('NUM_STAGES', None) or config.num_stages or '?'
        cfg_str = (f"BLOCK_M={config.kwargs.get('BLOCK_M','?')}, "
                   f"BLOCK_N={config.kwargs.get('BLOCK_N','?')}, stages={stages}")
        if verbose:
            status = "PASS" if passed else f"FAIL(max={max_diff:.2e})"
            print(f"    config {i}: {cfg_str} -> {status}", flush=True)

        if passed:
            accurate_configs.append((config, max_diff, cfg_str))

    _restore_configs(all_configs)

    if not accurate_configs:
        return None, None, float('inf'), "ALL_FAIL"

    best_tflops = 0
    best_result = None
    total_flops = compute_flops(B, HQ, M, N, D, causal)

    for config, max_diff, cfg_str in accurate_configs:
        _force_config(config)
        try:
            q, k, v, md = input_helper(B, HQ, HK, M, N, D, dtype, layout)
            if causal:
                md.need_causal()
            o = torch.empty_like(q)
            fn = lambda: run_gluon_attention(q, k, v, o, md)
            fn()
            torch.cuda.synchronize()
            ms = triton.testing.do_bench(fn, warmup=50, rep=200, return_mode='median')
            tflops = total_flops / ms * 1e-9
            if verbose:
                print(f"    bench {cfg_str} -> {tflops:.1f} TFLOPS", flush=True)
            if tflops > best_tflops:
                best_tflops = tflops
                best_result = (tflops, ms, max_diff, cfg_str)
        except Exception:
            pass

    _restore_configs(all_configs)
    return best_result if best_result else (None, None, float('inf'), "BENCH_FAIL")


def benchmark_single(B, HQ, HK, M, N, D, causal, layout, dtype, warmup=50, rep=200):
    """Benchmark a single configuration. Returns (tflops, ms)."""
    try:
        q, k, v, md = input_helper(B, HQ, HK, M, N, D, dtype, layout)
        if causal:
            md.need_causal()
        ...
        total_flops = compute_flops(B, HQ, M, N, D, causal)
        return total_flops / ms * 1e-9, ms
    except Exception as e:
        return None, str(e)


def run_sweep(configs, layout='bhsd', dtype=torch.float16, verbose=False,
              check=False, check_only=False):
    """Run benchmark sweep over a list of (B, HQ, HK, M, N, D, causal) configs."""
    results = []

    for i, cfg in enumerate(configs):
        B, HQ, HK, M, N, D, causal = cfg
        causal_str = "causal" if causal else "full"

        if verbose or check_only:
            print(f"[{i+1}/{len(configs)}] B={B}, H={HQ}/{HK}, Seq={M}x{N}, D={D}, {causal_str}...",
                  flush=True)

        tflops = ms = maxd = meand = cfg_str = None
        acc = None

        if check or check_only:
            tflops, ms, maxd, cfg_str = find_best_accurate_config(
                B, HQ, HK, M, N, D, causal, layout, dtype, verbose=verbose)
            acc = tflops is not None
            meand = 0.0

            if check_only:
                a_str = "PASS" if acc else "FAIL"
                d_str = f"{maxd:.2e}" if maxd != float('inf') else "ERR"
                print(f"  Gluon: {a_str} (max={d_str}, cfg={cfg_str})", flush=True)
                if check_only:
                    tflops = ms = None
        else:
            tflops, ms = benchmark_single(B, HQ, HK, M, N, D, causal, layout, dtype)

        results.append({
            'B': B, 'HQ': HQ, 'HK': HK, 'M': M, 'N': N, 'D': D,
            'causal': causal,
            'tflops': tflops, 'ms': ms,
            'acc': acc, 'maxd': maxd, 'meand': meand,
            'cfg': cfg_str,
        })
...
# ---------------------------------------------------------------------------
# Benchmark configuration suites
# ---------------------------------------------------------------------------
# Each entry: (Batch, HeadsQ, HeadsK, SeqQ, SeqK, D_HEAD, Causal)

QUICK_CONFIGS = [
    (2, 48, 48, 4096, 4096, 128, False),
]

PIPELINED_TEST_CONFIGS = [
    (1, 4, 2, 16384, 16384, 128, False),
    (1, 4, 2, 16384, 16384, 128, True),
]

D64_TEST_CONFIGS = [
    (2, 48, 48, 1024, 1024, 64, False),
    (2, 48, 48, 2048, 2048, 64, False),
    (2, 48, 48, 4096, 4096, 64, False),
    (2, 48, 48, 8192, 8192, 64, False),
    (2, 48, 48, 4096, 4096, 64, True),
    (2, 48, 48, 8192, 8192, 64, True),
    (1, 16, 16, 16384, 16384, 64, False),
    (1, 16, 16, 16384, 16384, 64, True),
    (2, 48, 24, 4096, 4096, 64, False),
    (1, 4,  2,  16384, 16384, 64, False),
]

STANDARD_CONFIGS = [
    (2, 48, 48, 1024,  1024,  128, False),
    (2, 48, 48, 2048,  2048,  128, False),
    (2, 48, 48, 4096,  4096,  128, False),
    (2, 48, 48, 8192,  8192,  128, False),
    (1, 16, 16, 16384, 16384, 128, False),
    (2, 48, 48, 4096,  4096,  128, True),
    (2, 48, 48, 8192,  8192,  128, True),
    (1, 16, 16, 16384, 16384, 128, True),
    (2, 48, 24, 4096,  4096,  128, False),
    (2, 48, 12, 4096,  4096,  128, False),
    (2, 48, 48, 4096,  4096,  64,  False),
    (2, 48, 48, 8192,  4096,  128, False),
    (2, 48, 48, 4096,  8192,  128, False),
]

EXTENDED_CONFIGS = STANDARD_CONFIGS + [
    (4,  48, 48, 2048, 2048, 128, False),
    (8,  16, 16, 2048, 2048, 128, False),
    (2,  64, 64, 4096, 4096, 128, False),
    (2,  96, 96, 2048, 2048, 128, False),
]

COMPREHENSIVE_CONFIGS = STANDARD_CONFIGS + PIPELINED_TEST_CONFIGS

REAL_WORLD_CONFIGS = [
    # Llama-3-8B / Qwen3-8B: 32Q / 8KV, D=128
    (1, 32, 8,  4096,  4096,  128, False),
    (1, 32, 8,  8192,  8192,  128, False),
    (1, 32, 8,  16384, 16384, 128, False),
    (1, 32, 8,  4096,  4096,  128, True),
    (1, 32, 8,  16384, 16384, 128, True),
    # Llama-3-70B / Qwen3-32B: 64Q / 8KV
    (1, 64, 8,  4096,  4096,  128, False),
    (1, 64, 8,  8192,  8192,  128, False),
    (1, 64, 8,  16384, 16384, 128, False),
    (1, 64, 8,  8192,  8192,  128, True),
    # Llama 4 Maverick / Qwen3-14B: 40Q / 8KV
    (1, 40, 8,  4096,  4096,  128, False),
    (1, 40, 8,  8192,  8192,  128, False),
    (1, 40, 8,  8192,  8192,  128, True),
    # Llama-3-405B: 128Q / 8KV
    (1, 128, 8, 4096,  4096,  128, False),
    (1, 128, 8, 8192,  8192,  128, False),
    # Batched serving
    (4, 32, 8,  4096,  4096,  128, False),
    (4, 64, 8,  4096,  4096,  128, False),
]
...
def main():
    parser = argparse.ArgumentParser(
        description="Standalone Gluon FA benchmark (gfx950 / MI350)")
    parser.add_argument('--quick',         action='store_true', help="Single config, fast run")
    parser.add_argument('--extended',      action='store_true', help="Extended config sweep")
    parser.add_argument('--pipelined',     action='store_true', help="Pipelined (D>=128) test cases")
    parser.add_argument('--comprehensive', action='store_true', help="Standard + pipelined configs")
    parser.add_argument('--d64',           action='store_true', help="D=64 head-dim sweep")
    parser.add_argument('--realworld',     action='store_true', help="Real-world LLM shapes (GQA)")
    parser.add_argument('--verbose',       action='store_true', help="Per-config progress output")
    parser.add_argument('--check',         action='store_true', help="Accuracy-gated benchmark")
    parser.add_argument('--check-only',    action='store_true', help="Accuracy check, skip TFLOPS")
    ...
    if args.quick:
        ...
        label = "QUICK (1 config)"
    elif args.extended:
        configs = EXTENDED_CONFIGS
        label = f"EXTENDED ({len(EXTENDED_CONFIGS)} configs)"
    elif args.pipelined:
        configs = PIPELINED_TEST_CONFIGS
        label = f"PIPELINED ({len(PIPELINED_TEST_CONFIGS)} configs)"
    elif args.comprehensive:
        configs = COMPREHENSIVE_CONFIGS
        label = f"COMPREHENSIVE ({len(COMPREHENSIVE_CONFIGS)} configs)"
    elif args.d64:
        configs = D64_TEST_CONFIGS
        label = f"D=64 ({len(D64_TEST_CONFIGS)} configs)"
    elif args.realworld:
        configs = REAL_WORLD_CONFIGS
        label = f"REAL-WORLD ({len(REAL_WORLD_CONFIGS)} configs)"
    else:
        configs = STANDARD_CONFIGS
        label = f"STANDARD ({len(STANDARD_CONFIGS)} configs)"

    print(f"Running {label} | layout={args.layout} dtype={args.dtype}")
    print()

    check_only = getattr(args, 'check_only', False)
    results = run_sweep(configs, layout=args.layout, dtype=dtype,
                        verbose=args.verbose, check=args.check, check_only=check_only)
    print_results(results, check_only=check_only)

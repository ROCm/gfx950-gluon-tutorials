...
## Provenance
`LlirSchedPlugin.cpp` is generated by `gen_plugin.py` from the upstream
`sched.barrier` LLIR-scheduler pass (AMD-Triton `llir-scheduler-upstream`
branch): it strips the in-tree headers, inlines `isMFMAorWMMA`, and swaps the
legacy `FunctionPass` wrapper for a new-PM pass + `llvmGetPassPluginInfo`.

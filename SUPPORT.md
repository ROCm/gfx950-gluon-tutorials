...
The tutorial pins Triton to the [`gfx950-tutorial-v2.0`](https://github.com/triton-lang/triton/releases/tag/gfx950-tutorial-v2.0) annotated tag in `triton-lang/triton`. The GEMM performance numbers were measured on the earlier [`gfx950-tutorial-v1.1`](https://github.com/triton-lang/triton/releases/tag/gfx950-tutorial-v1.1) build and reproduce on `v2.0`, which compiles them to byte-identical assembly; the attention numbers are `v2.0`-only, since `fav4` does not build without it. the committed IR/assembly dumps predate the v1.1 re-pin and are reproduced against [`gfx950-tutorial-v1.0`](https://github.com/triton-lang/triton/releases/tag/gfx950-tutorial-v1.0) (regeneration against v1.1 is pending). Both tags are immutable — they will not be moved or deleted. Building Triton from the relevant tag (or any commit reachable from it) reproduces the measurements within run-to-run noise.
